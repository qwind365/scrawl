...
class DrawPath: NSObject {
    ...
    func getDrawRect() -> (CGPoint,CGPoint) {
        return (CGPoint.init(x: self.startPoint.x - self.width / 2, y: self.startPoint.y - self.width / 2),CGPoint.init(x: self.endPoint.x + self.width / 2, y: self.endPoint.y + self.width / 2))
    }
}
